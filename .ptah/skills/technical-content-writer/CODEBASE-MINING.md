...
```bash
Read(.ptah/specs/TASK_XXXX/implementation-plan.md)
...
```bash
Read(.ptah/specs/TASK_XXXX/tasks.md)
...
```bash
Read(.ptah/specs/TASK_XXXX/code-*-review.md)
...
# Recent achievements
Read(.ptah/specs/registry.md)  # Completed tasks with metrics
```
...
Read(.ptah/specs/TASK_XXXX/implementation-plan.md)  # The solution

# Code examples
Read(libs/[library]/src/lib/services/*.service.ts)

# Results/metrics
Grep("Complete|DONE", .ptah/specs/TASK_XXXX/tasks.md)
```
...
# Demo flows
Read(.ptah/specs/TASK_XXXX/context.md)  # User journey
...
### 1. Task Registry (.ptah/specs/registry.md)

- Complete history of all features
- Status of each feature
- Task IDs for deep dives

### 2. Context Files (.ptah/specs/TASK_XXXX/context.md)
...
```bash
# Completed tasks
Grep("Complete", .ptah/specs/registry.md)

# Features by type
Grep("FEATURE|BUGFIX|REFACTOR", .ptah/specs/registry.md)

# Timeline
Grep("Created.*2025", .ptah/specs/registry.md)
```
...
Grep("Description", .ptah/specs/registry.md)
```
