...
{
    public decimal ForTier(WinTier tier, EngineConfig? config = null)
    {
        var cfg = config ?? EngineConfig.Default;
        return tier switch
        {
            WinTier.Small => decimal.Round(EffectiveScale * cfg.SmallTierFactor, 4),
            WinTier.Medium => decimal.Round(EffectiveScale * cfg.MediumTierFactor, 4),
            WinTier.Big => decimal.Round(EffectiveScale * cfg.BigTierFactor, 4),
            _ => EffectiveScale
        };
    }
}
