...
param(
    ...
    [string]$OutputPath = 'artifacts/godot-android/dev/Lucky5Cabinet.apk',
...
if ($Clean) {
    foreach ($path in @($outputFullPath, (Join-Path $outputDir 'godot-version.txt'), (Join-Path $outputDir 'build-info.json'))) {
...
if (-not (Test-Path -LiteralPath $outputFullPath)) {
    throw "Android export did not create an APK: $outputFullPath"
}

$apk = Get-Item -LiteralPath $outputFullPath
$apkHash = Get-FileHash -Algorithm SHA256 -LiteralPath $outputFullPath
Set-Content -LiteralPath (Join-Path $outputDir 'godot-version.txt') -Value ($godotVersion -join "`n") -Encoding utf8NoBOM

$buildInfo = [ordered]@{
    schema_version = 'lucky5.godot_android_export.v1'
    ...
    package_name = 'com.lucky5.cabinet'
    status = 'passed'
}
