...
public static class CleanRoomEngineTests
{
    public static Task RunAsync(List<string> failures)
    {
        ...
        Assert(failures, "Approved payout-scale defaults should match the rebalanced architecture", defaultConfig.DefaultPayoutScale == 1.95m && defaultConfig.MinPayoutScale == 1.25m && defaultConfig.MaxPayoutScale == 2.35m);
