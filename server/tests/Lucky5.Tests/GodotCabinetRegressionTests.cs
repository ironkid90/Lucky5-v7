...
public static class GodotCabinetRegressionTests
{
    public static async Task RunAsync(List<string> failures)
    {
        ...
        Assert(
            failures,
            "Godot cabinet double-up must use one visible deck row: dealer fixed first, live shuffle/result second, hit cards filling left-to-right after it",
            rootScript.Contains("const DU_BOARD_CARD_SIZE := Vector2(104, 146)", StringComparison.Ordinal)
                && rootScript.Contains("const DU_TRAIL_CARD_SIZE := Vector2(104, 146)", StringComparison.Ordinal)
                ...
                && rootScript.Contains("var hit_codes := _du_visible_hit_codes(du_data, dealer_code, challenger_code, max(0, du_cards.size() - 2))", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_board_slot(0, dealer_code, \"DEALER\", _is_lucky_du_card(du_data, dealer_code))", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_dealer_slot(0)", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_challenger_slot(1)", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_board_slot(1, challenger_code, _du_result_label(status), _is_lucky_du_card(du_data, challenger_code))", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_board_back(1, \"BIG / SMALL ?\", 1.0)", StringComparison.Ordinal)
                && rootScript.Contains("for slot_index in range(2, du_cards.size()):", StringComparison.Ordinal)
                && rootScript.Contains("var hit_index := slot_index - 2", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_board_slot(slot_index, hit_code, \"HIT\", _is_lucky_du_card(du_data, hit_code))", StringComparison.Ordinal)
                ...
                && rootScript.Contains("if dealer_replace_only:", StringComparison.Ordinal)
        ...
        Assert(
            ...
            rootScript.Contains("var hold_buttons: Array = []", StringComparison.Ordinal)
                ...
                && rootScript.Contains("return _is_double_up_active(du) and _du_switches_remaining(du) > 0", StringComparison.Ordinal)
                ...
                && rootScript.Contains("if id == \"bet\" and _can_switch_double_up_dealer(): return true", StringComparison.Ordinal)
        ...
        Assert(
            ...
            rootScript.Contains("var paytable_rows: Dictionary = {}", StringComparison.Ordinal)
                ...
                && rootScript.Contains("full_house_jackpot_label.text = _format_amount(_du_first_value(jp, [\"full_house\", \"fullHouse\", \"FullHouse\"], 0))", StringComparison.Ordinal)
                && rootScript.Contains("_refresh_paytable_values()", StringComparison.Ordinal)
                && rootScript.Contains("_refresh_paytable_highlights()", StringComparison.Ordinal));
