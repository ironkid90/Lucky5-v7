...
public static class GodotCabinetRegressionTests
{
    public static async Task RunAsync(List<string> failures)
    {
        ...
        Assert(
            failures,
            "Godot cabinet double-up must use one visible deck row: current dealer in the revealed trail, shuffle in the next slot, no separate winning hand/stage",
            rootScript.Contains("const DU_BOARD_CARD_SIZE := Vector2(92, 129)", StringComparison.Ordinal)
                && rootScript.Contains("const DU_TRAIL_CARD_SIZE := Vector2(92, 129)", StringComparison.Ordinal)
                ...
                && rootScript.Contains("var du_trail_anchor_code := \"\"", StringComparison.Ordinal)
                && rootScript.Contains("var switch_replaced_dealer := challenger_code.is_empty() and dealer_changed and _prev_switches_remaining >= 0 and switches < _prev_switches_remaining", StringComparison.Ordinal)
                ...
                && rootScript.Contains("du_trail_anchor_code = dealer_code", StringComparison.Ordinal)
                && rootScript.Contains("du_trail_anchor_code = \"\"", StringComparison.Ordinal)
                && rootScript.Contains("_start_du_card_shuffle(dealer_code, \"\")", StringComparison.Ordinal)
                ...
                && rootScript.Contains("if not du_trail_anchor_code.is_empty():", StringComparison.Ordinal)
                && rootScript.Contains("var anchor_index := _du_first_code_index(result, du_trail_anchor_code)", StringComparison.Ordinal)
                && rootScript.Contains("func _du_first_code_index(codes: Array, target_code: String) -> int:", StringComparison.Ordinal)
                ...
                && rootScript.Contains("_set_du_board_back(index, \"BIG / SMALL ?\", 1.0)", StringComparison.Ordinal)
                && rootScript.Contains("_set_du_challenger_slot(index)", StringComparison.Ordinal)
        ...
        Assert(
            ...
            rootScript.Contains("var hold_buttons: Array = []", StringComparison.Ordinal)
                ...
                && rootScript.Contains("if store.game_state() == \"double_up\" and store.can_press(\"double_up_switch\"):", StringComparison.Ordinal)
